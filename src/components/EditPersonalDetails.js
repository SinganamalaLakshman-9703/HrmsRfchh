...
const EditFamilyDetails = ({ member, onSave, onCancel }) => {
  ...
  const validateForm = () => {
    ...
    const phoneLengthByCountry = {
      "+91": 10, // India
      "+1": 10, // USA
      "+44": 10, // UK (usually 10 digits without the leading 0)
      "+61": 9, // Australia
      "+64": 9, // New Zealand
      "+27": 9, // South Africa
      "+977": 10, // Nepal
    ...
    if (!formValues.phoneNumber) {
      ...
      const selectedCountryCode = formValues.countryCode;
      const phoneLength = phoneLengthByCountry[selectedCountryCode] || 10;

      // Check if phone number matches the expected length for the country
      const regex = new RegExp(`^[0-9]{${phoneLength}}$`);
      if (!regex.test(formValues.phoneNumber)) {
        validationErrors.phoneNumber = `Must be ${phoneLength} digits for ${selectedCountryCode}.`;
      } else if (/^0+$/.test(formValues.phoneNumber)) {
        validationErrors.phoneNumber = "Phone number cannot be all zeros.";
      } else if (/^0/.test(formValues.phoneNumber)) {
        validationErrors.phoneNumber = "Number should not start with '0'.";
      }
    }
    // Marital Status
