...
const NationalIDDetails = () => {
  ...
  useEffect(() => {
    const fetchNationalID = async () => {
        try {
            const response = await axios.get(`http://192.168.0.119:8080/employeeservice/nationalID/getNationalID/HRMS2`);
...
const handleEnter = (e)=>{
    ...
    if (Object.keys(errors).length === 0) {
      ...
      try {
        ...
        if (isEditMode) {
          ...
          await axios.patch('http://192.168.0.119:8080/employeeservice/nationalID/updateNationalID', pay);
          

          updatedTableData = updatedTableData.map((item, index) =>
            index === formData.index ? { ...pay } : item
          );
        } else {

          await axios.post('http://192.168.0.119:8080/employeeservice/nationalID/createNationalID', {
            ...pay,
            employeeId: formData.employeeId 
          });
          

          updatedTableData.push({ ...pay });
        }
      

        updatedTableData.sort((a, b) => b.primary - a.primary);
  ...
  //         primary: formData.isPrimaryID,
  //         unid: formData.unid || '', 
  //         employeeId: formData.employeeId || 'HRMS2'  
  //       };
  ...
  //         updatedTableData = updatedTableData.map((item, index) =>
  //           item.unid === formData.unid ? { ...pay, isPrimaryID: formData.isPrimaryID } : item
  //         );
  //       } else {
  //         // POST request to create a new national ID
  //         await axios.post(`http://192.168.0.119:8080/employeeservice/nationalID/createNationalID?employeeId=HRMS2`, pay);
  //         updatedTableData.push({ ...pay, isPrimaryID: formData.isPrimaryID });
  //       }
